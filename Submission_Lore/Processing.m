classdef Processing
    ...
    methods
        ...
        function trial_mx = trialMatrix(~, trials)
        ...
        function labels_matrix  = get_data_matrix_initial(~, trial)

            labels_matrix = zeros(size(trial,1), size(trial,2), size(trial(1,1).handPos, 1)-1, 1);
            for trial_n = 1:size(trial,1)
                for angle_n = 1: size(trial,2)
                    labels_matrix(trial_n, angle_n, :, :) = trial(trial_n, angle_n).handPos(1:2, 1);
                end
            end
        end
